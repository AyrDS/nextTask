...
export const Searcher = () => {
   ...
   const filteredProjects = search === '' ? [] : projects.filter(project => project.name.toLowerCase().includes(search.toLocaleLowerCase()));
         ...
         <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSearcher}>
            ...
            >
               <Combobox
                  ...
                  onChange={(project => (window.location = `/proyectos/${project._id}`))}
               >
                     ...
                     <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                        {
                           filteredProjects.map(project => (
                              <Combobox.Option
                                 ...
                                 value={project}
                                 className={
                                    ({ active }) => classNames('cursor-default select-none px-4 py-2', active && 'bg-sky-600 text-white')
                                 }
                              >
                                 {project.name}
                              </Combobox.Option>
